...
```{r}
acled_quarter <- acled_sf_mutated %>% 
...
```{r}
summary(acled_quarter_ppp)
```

```{r}
acled_quarter_ppp <- rjitter(acled_quarter_ppp, 
                                 retry=TRUE, 
                                 nsim=1, 
                                 drop=TRUE)
```
...
```{r}
any(duplicated(acled_quarter_ppp))
```

### 7.1.2 Including owin object

```{r}
acled_quarter_owin <- acled_quarter_ppp[myanmar_owin]
...
```{r}
# acledmyanmar_kde <- spattemp.density(acled_quarter_owin)
# summary(acledmyanmar_kde)
```

```{r}
# plot(acledmyanmar_kde)
```
